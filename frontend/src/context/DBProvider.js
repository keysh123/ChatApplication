...
const DBProvider = ({ children, loading, setLoading }) => {
  ...
  async function _getChats(id) {
    try {
      const res = await fetch(`http://localhost:4000/api/v1/chat-room/chat/${id}`, {
        credentials: "include",
      });
      if (res.ok) {
  ...
  async function _getChatRoom(id) {
    try {
      const res = await fetch(`http://localhost:4000/api/v1/chat-room/${id}`, {
        credentials: "include",
      });
      if (res.ok) {
        const data = await res.json();
    ...
    try {
      ...
      getRequest.onsuccess = async function (event) {
        ...
        if (room) {
          ...
          let x = await _getChatRoom(roomId);
          x.latestChatTime = time;
          x.countUnread = parseInt(1);
          const putRequest = objectStore.put(x);
